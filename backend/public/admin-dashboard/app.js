...
(() => {
  ...
  // BEGIN FACTORY RESET ADMIN TOKEN FIX V2
  ...
  function inspectAdminAccessToken(token) {
    const payload =
    ...
    const audiences =
      stringValues(payload.aud);

    const roles = [
      ...stringValues(payload.role),
      ...stringValues(payload.admin_role),
    ...
    const hasAdminIdentity =
      payload.admin_user_id !== undefined ||
      payload.adminUserId !== undefined ||
  ...
  function collectBundles(
    ...
    if (access.value || refresh.value) {
      bundles.push({
        ...
        directAccessRoot: null,
        directRefreshRoot: null
      });
    }
  ...
  function directTokenRoots(roots) {
    const accessRoots = [];
    const refreshRoots = [];
    ...
    for (const root of roots) {
      ...
      const key =
        root.key.toLowerCase();

      if (
        key.includes("refresh") &&
        key.includes("token")
      ) {
        refreshRoots.push(root);
        continue;
      }
  ...
  function findAdminSession() {
    ...
    const {
      accessRoots,
      refreshRoots
    } = directTokenRoots(roots);

    accessRoots.forEach((accessRoot) => {
      const refreshRoot =
        matchingRefreshRoot(
          accessRoot,
          refreshRoots
      ...
      bundles
        ...
        .sort(
          (left, right) =>
            ...
            left.inspection.score
        );

    if (adminBundles.length) {
      return adminBundles[0];
    }

    const adminRefreshRoot =
      refreshRoots.find((root) =>
        root.key
          .toLowerCase()
          .includes("admin")
  ...
  function saveRotatedTokens(
    ...
    if (
      ...
      session.tokenObject[accessKey] =
        newAccessToken;

      session.tokenObject[refreshKey] =
        newRefreshToken;

      session.root.storage.setItem(
        session.root.key,
        JSON.stringify(
          session.root.parsed
        )
    ...
    if (session.directAccessRoot) {
      session
        .directAccessRoot
        .storage
        .setItem(
          session.directAccessRoot.key,
          newAccessToken
        );
    }

    if (session.directRefreshRoot) {
      session
        .directRefreshRoot
        .storage
        .setItem(
          session.directRefreshRoot.key,
          newRefreshToken
        );
    }
  ...
  function extractRotatedTokens(payload) {
    const data =
      payload?.data || {};
  ...
  async function refreshAdminSession(
    ...
    if (!session?.refreshToken) {
      throw new Error(
        "Admin session expired. Sign out and sign in again."
      );
    }
    ...
    if (
      ...
      throw new Error(
        "Admin authentication refresh returned an invalid session."
      );
    }
  ...
  async function validAdminSession() {
    ...
    if (!session) {
      throw new Error(
        "Admin session পাওয়া যায়নি। Sign out করে আবার Sign in করুন।"
      );
    }

    if (
      session.inspection &&
      !session.inspection.expired
    ) {
      return session;
    }
  ...
  async function sendFactoryRequest(
    method,
    endpoint,
    body,
    session
  ...
  // END FACTORY RESET ADMIN TOKEN FIX V2
