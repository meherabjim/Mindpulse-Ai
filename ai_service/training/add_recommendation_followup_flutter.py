...
current_ai_text = AI_SCREEN.read_text(
    encoding="utf-8"
)

if (
    "RecommendationFollowupScreen("
    in current_ai_text
):
    raise RuntimeError(
        "Recommendation follow-up is already "
        "integrated. Do not run this patch again."
    )
...
backup_dir = (
    ...
    / "backups"
    / (
        "recommendation_followup_flutter_"
...
class RecommendationSessionApiException
    implements Exception {
  const RecommendationSessionApiException(
    this.message,
  );
...
class RecommendationSessionService {
    ...
    final response =
      ...
      body: <String, dynamic>{
        ...
        'feedback_type':
            feedbackType,
        'feedback_note':
            feedbackNote?.trim().isEmpty == true
                ? null
                : feedbackNote?.trim(),
      },
    );
  ...
  Map<String, dynamic> _decodeResponse(
    ...
    try {
      ...
      throw const
          RecommendationSessionApiException(
...
class RecommendationFollowupScreen
  ...
  @override
  State<RecommendationFollowupScreen>
      createState() =>
...
class _RecommendationFollowupScreenState
  ...
  Future<_CompletionFeedback?>
      _collectFeedback() async {
    var afterMood =
        widget.beforeMood;
...
new_imports = [
    (
        "import '../../recommendations/screens/"
        "recommendation_followup_screen.dart';"
    ),
    (
        "import '../../safety/screens/"
...
if import_marker not in ai_text:
    import_marker = (
        "import 'package:flutter/material.dart';"
    )
...
if import_marker not in ai_text:
    raise RuntimeError(
        "A safe import marker was not found "
        "in ai_wellness_screen.dart."
    )


for import_line in new_imports:
    if import_line not in ai_text:
...
ai_text, replacement_count = (
    recommendation_pattern.subn(
...
if helper_marker not in ai_text:
    raise RuntimeError(
        "Journal method marker was not found."
    )
...
helper_source = r'''
  ...
  Widget _buildRecommendationCard(
    ...
    final durationSeconds =
        _recommendationDuration(
      category,
    );

    final isProfessionalSupport =
        category ==
...
print(
    "Recommendation follow-up Flutter "
