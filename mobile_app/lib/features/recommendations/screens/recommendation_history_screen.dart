...
class _RecommendationHistoryScreenState
    ...
    try {
      final responses = await Future.wait<Map<String, dynamic>>(
        <Future<Map<String, dynamic>>>[
          _service.getSummary(days: _days),
          _service.getHistory(page: 1, limit: 50),
        ],
      );
      ...
      final rawSessions = history['sessions'];
  ...
  List<Map<String, dynamic>> get _categories {
    final value = _summary['categories'];
  ...
  Widget _buildSummary() {
    final completed = _intValue(_summary['completed_sessions']);

    final total = _intValue(_summary['total_sessions']);

    final helpful = _intValue(_summary['helpful_sessions']);

    final notUseful = _intValue(_summary['not_useful_sessions']);

    final duration = _intValue(_summary['total_duration_seconds']);
    ...
    return Column(
      ...
      children: [
        ...
        ...categories.map((category) {
          final name = category['category']?.toString() ?? 'general';

          final total = _intValue(category['total']);

          final completed = _intValue(category['completed']);

          final helpful = _intValue(category['helpful']);

          final averageSeconds = _intValue(
            category['average_completed_seconds'],
          );
  ...
  Widget _buildSessionCard(Map<String, dynamic> session) {
    ...
    final beforeMood = session['before_mood'];

    final afterMood = session['after_mood'];

    final beforeStress = session['before_stress'];

    final afterStress = session['after_stress'];

    final duration = _intValue(session['actual_duration_seconds']);

    final feedback = session['feedback_type']?.toString();
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text(
                        session['recommendation_title']?.toString() ??
                            'Wellness action',
                      ...
                      Text(
                        _dateTimeLabel(session['started_at']),
                        style: const TextStyle(fontSize: 12.5),
                      ),
                    ],
                  ),
            ...
            Wrap(
              ...
              children: [
                ...
                Chip(
                  ...
                  label: Text(_duration(duration)),
                ),
                Chip(label: Text(_feedbackLabel(feedback))),
              ],
            ),
            if (beforeMood != null && afterMood != null) ...[
            ...
            if (session['feedback_note']?.toString().trim().isNotEmpty ==
                true) ...[
              const SizedBox(height: 9),
              Text(
                session['feedback_note'].toString(),
